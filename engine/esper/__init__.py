...
def _make_callback(name: str, func: _Callable[..., None]):
...
def remove_handler(name: str, func: _Callable[..., None]) -> None:
    ...
    not exist, this function will pass silently.
    """
...
class Processor:
    ...
    For example:
        ...
        myprocessor_instance = MyProcessor()
    ...
    :py:func:`esper.process`. Insinde the `process` method is generally where you
    should iterate over Entities with one (or more) calls to the appropriate methods
    
        def process(self):
            for entity, (rend, vel) in esper.get_components(Renderable, Velocity):
                your_code_here()"""
...
def remove_processor(processor_type: _Type[Processor]) -> None:
    ...
    Make sure to provide the class itself, NOT an intance. For example::
...
def get_processor(processor_type: _Type[Processor]) -> None:
...
def create_entity(*components: _C) -> int:
    ...
    assigned to the Entity on creation. Components can also be
...
def delete_entity(entity: int, immediate: bool = False) -> None:
    ...
    Delete an Entity and all of its assigned Component instances from
    the world. By defauly, Entity deletion is delayed until the next call
...
def entity_exists(entity: int) -> bool:
    ...
    Empty Entities (with no component) and dead Entities(destroyed
    by delete_entity) will not count as existing ones.
    """
...
def component_for_entity(entity: int, component_type: _Type[_C]) -> _C:
    ...
    it ma be necessary to access a specific Component instance.
...
def clear_dead_entities() -> None:
    ...
    your main loop after caling all processors.
    """
...
def process(*args: _Any, **kwargs: _Any) -> None:
    ...
    that were marked for deletion dince the last call will be deleted 
...
def timed_process(*args: _Any, **kwargs: _Any) -> None:
    ...
    (in milliseconds) in the :py:attr:`~process_times` directory
    after each call.
    """
    clear_dead_entities()
    for processor in _processors:
...
def list_worlds() -> _List[str]:
    """List all World context names."""
    return list(_context_map)

def delete_world(name: str) -> None:
    """Delete a World context.
    
    This will completely delte the World, including all Entities
...
def switch_world(name: str) -> None:
    ...
    global _get_comnponents_cache
    global _processors
    global process_times
    global event_registry
    global current_world

    # switch the references to the objects in the names context_map:
